...
/**
 * Caminho do `.env` do app — fora do repositório versionado, ver
 * `.gitignore` da raiz.
 *
 * Deriva de `RAIZ_DO_BLOG` em vez de contar `../..` a partir de
 * `__dirname` neste arquivo: o electron-vite empacota todo o processo
 * principal num único `dist-electron/principal/index.js`, então
 * `__dirname` é o mesmo para todo módulo nele incluído — contar níveis
 * relativos a este arquivo específico dá o resultado errado (bug real,
 * encontrado ao testar contra o Notion de verdade: o `.env` era procurado
 * na raiz do repositório em vez de em `app/`).
 */
export const CAMINHO_ENV_PADRAO = join(RAIZ_DO_BLOG, 'app', '.env');
